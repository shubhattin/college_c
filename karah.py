...
def main():
    ...
    for x in re.findall(r"\d+\. .+?\[.+?\]\(.+?/\w+\.c\)", README, re.DOTALL):
        nm_str: str = re.findall(r"\[.+?\]\(.+?/\w+?\.c\)", x)[0]
        ...
        if os.path.isfile(x):
            ...
            for y in os.listdir(x):
                ...
                if re.match(r".+\.(pdf|docx|txt|md)$", y):
                    ...
                    continue
                if y[-6:] == ".class" or not y.endswith(".c"):
                    continue
                if not dir_made:
    ...
    sh.write(
        ...
        render(
            "home.html.j2",
            md_content=markdowninfy(README.replace(".c)", ".html)")),
            title=NM,
        ),
    )
    c += 1
